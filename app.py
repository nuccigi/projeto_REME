...
# ============================
def natural_key(s):
    # quebra em blocos dígitos/não-dígitos e converte dígitos para int
...
def class_geral_from_score(s: pd.Series) -> pd.Categorical:
    """
    Classificação geral fixa no intervalo 0–100:
    0–20 -> R1, 20–40 -> R2, ..., 80–100 -> R5
    """
...
def add_reclassificacoes(df_resultado: pd.DataFrame) -> pd.DataFrame:
    """
    Recebe o DataFrame de saída do calcular_scores_mensais (colunas: talhao, mes, score).
    Adiciona:
      - classe_geral (R1..R5) pela faixa fixa 0–100
      - *_idx numéricos (1..5) para facilitar gráficos
    """
...
def criar_mapa_pydeck(df_mapa, titulo_mapa, tooltip_html):
    ...
    # tooltip precisa estar no Deck (não no Layer)
    r = pdk.Deck(
...
if arquivo is not None:
    try:
        ...
        # ✅ Corrige o tooltip: deck.gl não aceita {score_medio:.1f}
    ...
    with col_mapa_mensal:
        ...
        if not df_risco_mensal.empty:
            ...
            # ✅ Corrige o tooltip: arredonda antes e remove : .1f do HTML
